...
import type { InvestmentNormalized } from "@/types/investment/investment.types";
import {
...
const computeMonthlyCycle = (referenceDate?: string | Date) => {
  const now = new Date();
  const startOfCycle = firstOfMonth(now);
  const maturityDate = firstOfNextMonth(now);
  const lastDayOfThisMonth = lastOfMonth(now);
  ...
  const progressPct =
    daysInThisMonth > 0 ? (daysCompleted / daysInThisMonth) * 100 : 0;
...
export default function Dashboard() {
  ...
  // ---------- STABLE displayedUser (avoid flicker) ----------
  ...
  useEffect(() => {
    ...
    // keep deps minimal to avoid unnecessary resets
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [portfolio]);

  // ---------- STABLE investments list (prevents empty-flash on refetch) ----------
  const [investments, setInvestments] = useState<InvestmentNormalized[]>([]);
  ...
  // Dashboard stats from displayedUser
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            ...
            className="gradient-primary text-white shadow-lg hover:shadow-xl transition-all duration-200 px-8 py-3 text-base font-medium rounded-xl"
          >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button className="border-border hover:shadow-lg  px-6 py-2 rounded-xl">
              View All Plans
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {investmentsLoading && investments.length === 0 ? (
            // Skeleton placeholders while first load
            ...
            investments.map((investment) => {
              // Replace previous duration-based timeline with monthly-cycle timeline
                  ...
                  <div className="relative z-10 p-6 space-y-6">
                    ...
                    <div className="space-y-4 p-4 bg-muted/20 rounded-xl border border-border/30">
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground pt-2 border-t border-border/30">
                        <span>
                          Started:{" "}
                          {format(investment.createdAt, "MMM dd, yyyy")}
                        </span>
                        <span>
