...
import { User, Investment, TransactionLog, Withdrawal, PendingPrincipalWithdrawal } from '@/types/investment';
...
const INITIAL_USER: User = {
  ...
  referralEarnings: 1842.30,    // Referral commissions
...
export function useInvestmentStore() {
  ...
  const [user, setUser] = useState<User>(() => {
    ...
    const withdrawalBlocked = localStorage.getItem('withdrawalBlocked') === 'true';
    return {
      ...INITIAL_USER,
  ...
  const [transactions, setTransactions] = useState<TransactionLog[]>(INITIAL_TRANSACTIONS);
  const [withdrawals, setWithdrawals] = useState<Withdrawal[]>(INITIAL_WITHDRAWALS);
  const [pendingPrincipalWithdrawals, setPendingPrincipalWithdrawals] = useState<PendingPrincipalWithdrawal[]>([]);
  ...
  const calculateProRatedReturn = (investment: Investment, targetMonth: Date) => {
    ...
    if (startDate.getMonth() === targetMonth.getMonth() && startDate.getFullYear() === targetMonth.getFullYear()) {
  ...
  const createInvestment = (planId: string, planName: string, amount: number, roi: number, name?: string) => {
    ...
    investments.forEach(investment => {
      if (investment.status === 'active') {
        ...
        if (returnAmount > 0) {
          ...
          setUser(prev => ({
            ...prev,
            investmentReturnBalance: prev.investmentReturnBalance + returnAmount
          }));
          ...
          setInvestments(prev => 
            prev.map(inv => 
              inv.id === investment.id 
                ? { 
                    ...inv, 
                    nextPayoutDate: firstOfNextMonth.toISOString().split('T')[0],
          ...
          const transaction: TransactionLog = {
            ...
            date: todayString,
            status: 'success',
            description: `Monthly Return (${investment.roi}%) - ${investment.name || investment.planName} (${today.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })})`,
  ...
  const processPendingPrincipalWithdrawals = () => {
    ...
    pendingPrincipalWithdrawals
      .filter(pending => pending.status === 'pending' && pending.processDate <= today)
      .forEach(pending => {
  ...
  const closeMatureInvestment = (investmentId: string) => {
    ...
    const transaction: TransactionLog = {
      ...
      date: new Date().toISOString().split('T')[0],
      status: 'pending',
      description: `Investment Closed - Principal processing (15 days) - ${investment.name || investment.planName}`,
      relatedId: investment.id
    };
      ...
      prev.map(w => 
        w.id === withdrawalId 
          ? { 
              ...
              verificationDeadline: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
            }
          : w
      )
    );
  ...
  const completeWithdrawalVerification = (withdrawalId: string) => {
    ...
    if (isApproved) {
      setWithdrawals(prev => 
        prev.map(w => 
          w.id === withdrawalId 
            ? { ...w, status: 'approved' as const, processedDate: new Date().toISOString().split('T')[0] }
            : w
        )
      );
      ...
      if (isBlocked) {
        ...
        toast({
          ...
          description: "Maximum verification attempts exceeded. Contact support for assistance.",
          variant: "destructive"
        });
      } else {
        toast({
  ...
  const createWithdrawal = (amount: number, source: "return" | "referral" | "principal", walletAddress: string, twoFactorCode?: string) => {
    ...
    if (twoFactorCode !== "123456") { // Mock validation - accept 123456 for demo
      toast({
    ...
    const pendingUploads = withdrawals.filter(w => 
      w.status === "admin_approved" && !w.screenshotUrl
    ).length;
    
    if (pendingUploads >= 3) {
      return { success: false, error: "You have 3 or more withdrawals pending screenshot upload. Please upload screenshots for previous withdrawals before creating new ones." };
    }
    
    const availableBalance = source === "return" 
    ...
    if (amount > availableBalance || amount < 10) {
      toast({
        ...
        description: amount > availableBalance ? "Insufficient balance" : "Minimum withdrawal is $10",
        variant: "destructive"
      });
      return { success: false, error: amount > availableBalance ? "Insufficient balance" : "Minimum withdrawal is $10" };
    }
    ...
    setUser(prev => ({
      ...prev,
      ...(source === "return" && { investmentReturnBalance: prev.investmentReturnBalance - amount }),
      ...(source === "referral" && { referralEarnings: prev.referralEarnings - amount }),
      ...(source === "principal" && { walletBalance: prev.walletBalance - amount })
    }));
    ...
    const transaction: TransactionLog = {
      ...
      date: new Date().toISOString().split('T')[0],
      status: 'pending',
      description: `Withdrawal Request - ${source === "return" ? "Investment Returns" : source === "referral" ? "Referral Earnings" : "Investment Principal"}`,
